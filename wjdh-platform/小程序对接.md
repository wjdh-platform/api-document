...
- [1授权](#1授权)
  - [1.1获取手机验证码](#11获取手机验证码)
    ...
    - [1.1.4接口附录](#114接口附录)
  - [1.2注册](#12注册)
    - [1.2.1手机号注册+登陆二合一](#121手机号注册登陆二合一)
...
字段|格式|是否必填|名称|补充
|:--|:--|:--|:--|:--
code|integer|必填|处理结果|0成功，1出错，2在安防平台查找到了用户数据，自动进行了绑定
...
成功，用户不但注册和登陆，还自动绑定了一标三实数据
{
    "code": "2",
    "token_expired": "1",
    "msg": "已自动绑定您在安防平台录入的数据",
    "access_token": "eyJ0eXAiO",
    "token_type": "Bearer",
    "expires_in": 3600,
    "roles": [
...
shenhe_phone|string|选填|用户第一次录入一标三实时使用的手机号码
...
>url地址：<https://tc.mg.cool/api/v1/people/roles>
...
名称|释义|权限|补充
|:--|:--|:--|:--
新用户|NewMember|暂未设置|该角色未拥有shenhe_phone
第一次录入一标三实的用户（待审核状态）|Shenheing|暂未设置|该角色已经拥有shenhe_phone
已绑定用户|HouseMember|暂未设置|该角色可以使用目前的所有功能模块
...
sex|integer|必填|性别|1男2女|扫描的有，手动填写的无
...
message|string|必填|调试信息|
token_expired|integer|必填|token值仍然未过期
shenhe_phone|string|必填|用户提交表单时使用的手机号，以后可以直接使用
