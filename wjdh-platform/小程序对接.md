...
- [1授权](#1授权)
  ...
  - [1.4获取token](#14获取token)
    ...
    - [1.5.1发送旧token值到后台获取新token](#151发送旧token值到后台获取新token)
...
``` json
1.成功，将通过手机号查找到的安防平台数据保存到了万家灯火数据库
{
    "code": 0,
    "msg": "已自动绑定您的一标三实信息",
    "message": "在安防平台查找到了数据"
...
``` json
2.成功，该用户无需再录入个人信息（people表中已经保存过），因此这个时候隐藏个人信息的录入部分即可。并且在绑定中增加传参——（更多房屋绑定的模式）详见表单提交接口。[理论上不会出现该结果，因为前端可以通过用户的HouseMember身份，来阻止用户使用该接口]
{
    code: "0"
